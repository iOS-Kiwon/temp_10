...
- (CGFloat)alphaForItemWithOffset:(CGFloat)offset
{
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeMochaMainStyle:
        {
            //do nothing
        }
    }
...
- (CGFloat)decelerationDistance
{
    ...
    if(_type == iCarouselTypeMochaMainStyle){
        if(_startVelocity > 0){
            if(_startVelocity < 5.000000f){
                _startVelocity = 5.000000f;
            }
        }else {
            if(_startVelocity > -5.000000f){
                _startVelocity = -5.000000f;
        ...
        //NSLog(@"벨로시티 %f ===== startVelocity : %f ",    -powf(_startVelocity, 2.0f) / (2.0f * acceleration) , _startVelocity);
        //return -powf(_startVelocity, 2.0f) / (2.0f * acceleration);
        if(-powf(_startVelocity, 2.0f) / (2.0f * acceleration) > 0.05000) {
            return 0.250000f;
            
        }else if(-powf(_startVelocity, 2.0f) / (2.0f * acceleration) <  -0.050000) {
            return -0.250000f;
        }else {
            return -powf(_startVelocity, 2.0f) / (2.0f * acceleration);
        }
