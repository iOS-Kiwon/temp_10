...
@interface AmailFMDatabasePool()

- (void)pushDatabaseBackInPool:(AmailFMDatabase*)db;
- (AmailFMDatabase*)db;

@end


@implementation AmailFMDatabasePool
...
- (id)initWithPath:(NSString*)aPath {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"AmailFMdb.%@", self] UTF8String], NULL);
        _databaseInPool     = AmailFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = AmailFMDBReturnRetained([NSMutableArray array]);
    }
...
- (void)dealloc {
    ...
    AmailFMDBRelease(_databaseInPool);
    AmailFMDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        AmailFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(AmailFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The AmailFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (AmailFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [AmailFMDatabase databaseWithPath:_path];
        }
