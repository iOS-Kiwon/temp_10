...
#import "SBJsonBase.h"  // nami0342 - SBJSON
@interface AmailSBJsonWriter ()
...
- (NSString*)indent;
@end
@implementation AmailSBJsonWriter
...
- (NSString*)stringWithFragment:(id)value {
    ...
    if ([self appendValue:value into:json]) return json;
    return nil;
}
- (NSString*)stringWithObject:(id)value {
    if ([value isKindOfClass:[NSDictionary class]] || [value isKindOfClass:[NSArray class]]) return [self stringWithFragment:value];
...
- (BOOL)appendValue:(id)fragment into:(NSMutableString*)json {
    if ([fragment isKindOfClass:[NSDictionary class]]) {
        if (![self appendDictionary:fragment into:json]) return NO;
    } else if ([fragment isKindOfClass:[NSArray class]]) {
        if (![self appendArray:fragment into:json])return NO;
    } else if ([fragment isKindOfClass:[NSString class]]) {
        if (![self appendString:fragment into:json]) return NO;
    } else if ([fragment isKindOfClass:[NSNumber class]]) {
        if ('c' == *[fragment objCType]) [json appendString:[fragment boolValue] ? @"true" : @"false"];
        else [json appendString:[fragment stringValue]];
...
- (BOOL)appendString:(NSString*)fragment into:(NSMutableString*)json {
    ...
    if ( !esc.length ) [json appendString:fragment];
    else {
