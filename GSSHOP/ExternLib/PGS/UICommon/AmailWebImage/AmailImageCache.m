...
static AmailImageCache *instance;
@implementation AmailImageCache
- (id)init
{
    if ((self = [super init]))
    {
...
#pragma mark AmailImageCache (class methods)
+ (AmailImageCache *)sharedImageCache
{
    if (instance == nil)
    {
        instance = [[AmailImageCache alloc] init];
    }
    return instance;
}
#pragma mark AmailImageCache (private)
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    id <AmailImageCacheDelegate> delegate = [arguments objectForKey:@"delegate"];
...
- (void)changeImageKey:(NSString *)oldKey newKey:(NSString *)newKey {
    ...
    UIImage *temp = [self imageFromKey:oldKey fromDisk:YES] ;
    [self removeImageForKey:oldKey];
    [self storeImage:temp forKey:newKey];
}
...
{
    [self storeImage:image 
           imageData:toDisk ? UIImageJPEGRepresentation(image, 1.0) : nil 
              forKey:key 
              toDisk:toDisk];
}
